...
func integrationStores(t *testing.T) (context.Context, *Store, *Store) {
	...
	if _, err := first.pool.Exec(ctx, `TRUNCATE command_attempts, command_outbox, command_events, commands, mission_deployments, mission_items, missions, flight_records, aircraft, received_peer_notifications, peer_notifications, operational_intent_publications, conflict_findings, operational_volumes, operational_intents`); err != nil {
		t.Fatal(err)
	}
