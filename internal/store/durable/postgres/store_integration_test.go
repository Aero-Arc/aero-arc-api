...
func TestAuthoritativeSpatialReadCheckSlice(t *testing.T) {
	...
	if _, err := store.pool.Exec(ctx, `TRUNCATE mission_deployments, mission_items, missions, received_peer_notifications, peer_notifications, operational_intent_publications, conflict_findings, operational_volumes, operational_intents`); err != nil {
		t.Fatal(err)
	}
...
func integrationStores(t *testing.T) (context.Context, *Store, *Store) {
	...
	if _, err := first.pool.Exec(ctx, `TRUNCATE mission_deployments, mission_items, missions, received_peer_notifications, peer_notifications, operational_intent_publications, conflict_findings, operational_volumes, operational_intents`); err != nil {
		t.Fatal(err)
	}
